...
export default function AiPlannerResult() {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      if (!preferences) {
        // Fallback if no preferences are passed
        setIsLoading(false);
        return;
      }
      try {
  ...
  if (isLoading) {
    return <MathCurveLoader message="Building your hyper-personalized AI itinerary..." />;
  }

  if (!plan) {
    return (
      <div className="page" style={{ textAlign: 'center', justifyContent: 'center' }}>
        <p>Could not generate a plan. Please try again.</p>
