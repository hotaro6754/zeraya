...
export default function Events() {
  ...
  const { user, events, dataLoading, db } = useFirebase();
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-secondary)' }}>
                    <Calendar size={14} />
                    <span className="body-sm">{e.date?.seconds ? new Date(e.date.seconds * 1000).toLocaleDateString() : e.date}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-secondary)' }}>
                    <MapPin size={14} />
